...
app.post("/upload", upload.single("video"), (req: Request, res: Response) => {
  ...
  ffprobe.on("close", async () => {
      ...
      for (const line of lines) {
        if (line.startsWith("out_time_ms=")) {
          ...
          if (totalSeconds > 0 && ms > 0) {
            const pct = Math.min(
              99,
              Math.round((ms / 1_000_000 / totalSeconds) * 100),
            );
    ...
    ffmpeg.on("close", (code: number | null) => {
      if (code === 0) {
        const sizeBytes = fs.existsSync(outputPath)
        ...
        updateJob(jobId, { status: "error", error: "FFmpeg failed." });
        cleanupJob(jobId);
      }
    });
...
// DELETE /delete/:id - cancels processing and deletes files
...
app.listen(PORT, () =>
  console.log(
    `Rezip running on http://localhost:${PORT}  [encoder: ${encoderProfile.encoder}]`,
  ),
);
